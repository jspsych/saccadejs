...
| `showVideo()` / `hideVideo()`                               | Show/hide a small mirrored camera preview in the bottom-left corner. Hiding keeps the `<video>` in the document, invisible at 2×2 px: Chrome delivers camera frames only to a rendered video, so `display: none` would stop the tracker.                                                                                                      |
...
| `calibratePoint(x, y, embeddings?, captureMs?, timeoutMs?)` | Add a calibration point at viewport pixel `(x, y)`. Without `embeddings`, collects them from the camera for `captureMs` (default 500) first. Rejects with `no camera frames for <ms> ms` if a single frame takes longer than `timeoutMs` (default 5000), rather than waiting forever on a dead camera. Returns the number of embeddings used. |
| `fitCalibration(lambda?)`                                   | Fit the ridge regression. Omit `lambda` to let the core pick with `lambdaFor(nPoints)`. Returns `{lambda, nPoints}` (distinct targets) or `null`.                                                                                                                                                                                             |
| `getCalibrationPoints()`                                    | The calibration points collected so far (targets in viewport fractions, 0–1).                                                                                                                                                                                                                                                                 |
| `getCurrentPrediction()`                                    | The latest `{x, y, t}` in viewport pixels, or `null`. `t` is absolute, as for `onGazeUpdate`.                                                                                                                                                                                                                                                 |
| `onGazeUpdate(cb)`                                          | Subscribe to gaze predictions. Returns a function that unsubscribes. `t` is always absolute (`performance.now()` scale, lag-corrected), never trial-relative like `saccade_data`'s.                                                                                                                                                           |
...
| `onSetupProgress(cb)`                                       | Subscribe to `start()`'s load progress (`SaccadeProgress`: camera → mediapipe → landmarker → ort → model → session → ready; only `model` reports bytes). The most recent report is replayed on subscribe. Returns a function that unsubscribes. A tracker supplied through the `tracker` parameter reports here too.                          |
