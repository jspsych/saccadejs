...
- Extra optional inputs: `SaccadeAssets.ortModuleUrl` / `mediapipeModuleUrl`; `SaccadeTrackerOptions.stream` (an existing `MediaStream`), `.model`, `.calHead` and `.onProgress`. Extra members: `tracker.onProgress(cb)` (load-progress subscription; replays the latest report, returns unsubscribe), `tracker.nextGaze()`, `getSmoothingFrames()`, `getKernel()`, `initialized`. Extra exports: `Pipeline`, `Landmarker` (with `createLandmarker({ headPose: true })` and `detectFace()` for the head-pose matrix), `headPoseFromMatrix`, `webglAvailable`, `WebGLUnavailableError` (thrown by `createLandmarker` before anything loads when there is no WebGL), `OrtEmbeddingModel`, `StubEmbeddingModel`, `fitRidge`, `CENTER`, `CAL_HEAD`, `mSequence`, `stimulusAt`, `refineLag`, `modelUrl`, `loadOrt`, `loadVision`, `DEFAULT_*_URL`.
...
9. **`t` from the live gaze API is always absolute (2026-09-14)**: `getCurrentPrediction()` and `onGazeUpdate` report `t = (time.meanCapture ?? time.capture) − (offset ?? 0)` on the `performance.now()` scale, whether or not the current trial has the extension attached. Only `saccade_data` rows are trial-relative (decision 1). Previously the live `t` switched to trial-relative while a recording trial was active, which broke `saccade-validate`'s `raw_gaze[i][j].t` (it subtracts its own start time) whenever the extension was attached to that trial.
